...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = [
        'Sedan', 'Bus or Truck', 'Motorcycle', 'Bicycle', 
        'Bicycle Group', 'Pedestrian', 'Pedestrian Group',
        'Background',
    ]
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = 0
        elif (current_cls_name == 'Sedan'.lower() and 'Bus or Truck'.lower() == gt_name):
            valid_class = 0
        else:
            valid_class = -1
        ignore = False
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    Args:
        gt_annos (dict): Must from get_label_annos() in kradar_common.py.
        dt_annos (dict): Must from get_label_annos() in kradar_common.py.
    ...
    # for debugging. TODO: remove this part
    # i = 0
    # dt = dt_annos[i]
    # gt = gt_annos[i]
    # dt_boxes = np.concatenate([dt['location'], dt['dimensions'], 
    #                            dt['rotation_y'][..., np.newaxis]], axis=1)
    # gt_boxes = np.concatenate([gt['location'], gt['dimensions'], 
    #                            gt['rotation_y'][..., np.newaxis]], axis=1)
...
def eval_class(gt_annos,
               ...
               compute_aos=False,
               num_parts=200):
    """kradar eval. support 2d/bev/3d/aos eval. support 0.5:0.05:0.95 coco AP.
    ...
    for m, current_class in enumerate(current_classes):
        for idx_l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # draw and save pr curve (bev)
                if metric > 0 and m ==0 and idx_l ==0:
                    metric_name = ['bev', '3d']
                    import matplotlib.pyplot as plt
                    dt_scores_list = [anno['score'] for anno in dt_annos]
                    dt_scores = np.concatenate(dt_scores_list, axis=0)
                    plt.hist(dt_scores, bins=40)
                    plt.xlabel('score')
                    plt.ylabel('count')
                    plt.title('score distribution')
                    # plt.savefig('./debug/vis/dt_score.png')

                    r = np.arange(0.0, 1.025, 0.025)
                    plt.figure()
                    plt.plot(r, precision[m, idx_l, k, :])
                    plt.xlabel('recall')
                    plt.ylabel('precision')
                    plt.ylim([0.0, 1.0])
                    plt.title('{}, class: sedan, min_overlap: {}'.format(
                        metric_name[metric-1], min_overlap))
                ...
                if metric > 0 and m ==0 and idx_l ==0:
                    plt.plot(r, precision[m, idx_l, k, :])
                    # plt.savefig('./debug/vis/{}_sedan_min_overlap_{}.png'.format(
                    #     metric_name[metric-1], min_overlap))
                    plt.close()
...
def kradar_eval(gt_annos,
    ...
    dict_ious = {
        'all': [overlap_easy, overlap_mod, overlap_hard],
        'hard': [overlap_hard],
        'mod': [overlap_mod],
        'easy': [overlap_easy]
    }
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            if mAP3d is not None:
                ret_dict[f'kradar/Overall_3D_{postfix}'] = mAP3d[idx, 0]
            if mAPbev is not None:
                ret_dict[f'kradar/Overall_BEV_{postfix}'] = mAPbev[idx, 0]
            if mAPbbox is not None:
                ret_dict[f'kradar/Overall_2D_{postfix}'] = mAPbbox[idx, 0]

    return result, ret_dict


def kradar_eval_coco_style(gt_annos, dt_annos, current_classes):
    """coco style evaluation of kradar.
